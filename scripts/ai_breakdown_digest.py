...
RSS_URL = "https://feeds.libsyn.com/468519/rss"

def fetch_latest():
    print("Fetching The AI Breakdown RSS...")
...
def generate_summary_from_audio(client, audio_file_path):
    ...
    prompt = """
You are an elite audio briefing announcer. Analyze the provided podcast audio from "The AI Breakdown" and create a highly condensed, intellectually dense 3-4 minute briefing script in English.
...
def main():
    ...
    state_file = os.path.join(os.path.dirname(__file__), "last_ai_breakdown_guid.txt")
    title, mp3_url, current_guid = fetch_latest()
    ...
    if current_guid and os.path.exists(state_file):
        ...
        if current_guid == last_guid:
            print("No new The AI Breakdown episode detected. Exiting.")
            return

    print(f"Latest Episode: {title}")
    
    local_audio = os.path.join(os.path.dirname(__file__), "temp_ai_breakdown.mp3")
    ...
    display_title = f"The AI Breakdown: {title}"
    full_text_for_feed = f"**{display_title}**\n\n" + summary_text
    
    import datetime
    date_str = datetime.datetime.now().strftime("%B %-d, %Y")
    spoken_text = f"This is The AI Breakdown. Episode of {date_str}. " + summary_text.replace("**", "")
