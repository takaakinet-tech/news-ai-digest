...
RSS_URL = "https://feeds.acast.com/public/shows/ft-news-briefing"

def fetch_latest():
    print("Fetching FT News Briefing RSS...")
...
def generate_summary_from_audio(client, audio_file_path):
    ...
    prompt = """
You are an elite audio briefing announcer. Analyze the provided podcast audio from "FT News Briefing" and create a highly condensed, intellectually dense 3-4 minute briefing script in English.
...
def main():
    ...
    state_file = os.path.join(os.path.dirname(__file__), "last_ft_news_briefing_guid.txt")
    ...
    if current_guid and os.path.exists(state_file):
        ...
        if current_guid == last_guid:
            print("No new FT News Briefing episode detected. Exiting.")
            return

    print(f"Latest Episode: {episode_title}")
    
    local_audio = os.path.join(os.path.dirname(__file__), "temp_ft_news_briefing.mp3")
    ...
    display_title = f"Financial Times FT News Briefing: {episode_title}"
    full_text_for_feed = f"**{display_title}**\n\n" + summary_text
    
    import datetime
    date_str = datetime.datetime.now().strftime("%B %-d, %Y")
    spoken_text = f"This is Financial Times, FT News Briefing. Episode of {date_str}. " + summary_text.replace("**", "")
