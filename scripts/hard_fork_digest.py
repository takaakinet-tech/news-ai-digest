...
RSS_URL = "https://feeds.simplecast.com/83p5Hn2n"

def fetch_latest():
    print("Fetching Hard Fork RSS...")
...
def generate_summary_from_audio(client, audio_file_path):
    ...
    prompt = """
You are an elite audio briefing announcer. Analyze the provided podcast audio from "Hard Fork" and create a highly condensed, intellectually dense 3-4 minute briefing script in English.
...
def main():
    ...
    state_file = os.path.join(os.path.dirname(__file__), "last_hard_fork_guid.txt")
    ...
    if current_guid and os.path.exists(state_file):
        ...
        if current_guid == last_guid:
            print("No new Hard Fork episode detected. Exiting.")
            return

    print(f"Latest Episode: {title}")
    
    local_audio = os.path.join(os.path.dirname(__file__), "temp_hard_fork.mp3")
    ...
    display_title = f"NYT Hard Fork: {title}"
    full_text_for_feed = f"**{display_title}**\n\n" + summary_text
    
    import datetime
    date_str = datetime.datetime.now().strftime("%B %-d, %Y")
    spoken_text = f"This is New York Times, Hard Fork. Episode of {date_str}. " + summary_text.replace("**", "")
