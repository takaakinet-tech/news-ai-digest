...
def fetch_nyt_latest():
    print("Fetching The Daily RSS...")
...
def main():
    ...
    state_file = os.path.join(os.path.dirname(__file__), "last_nyt_guid.txt")
    title, mp3_url, current_guid = fetch_nyt_latest()
    ...
    if current_guid and os.path.exists(state_file):
        ...
        if current_guid == last_guid:
            print("No new NYT episode detected. Exiting.")
            return

    print(f"Latest Episode: {title}")
    
    local_audio = os.path.join(os.path.dirname(__file__), "temp_nyt_daily.mp3")
