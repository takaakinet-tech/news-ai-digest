...
RSS_URL = "https://feeds.megaphone.fm/hbrideacast"

def fetch_latest():
    print("Fetching HBR IdeaCast RSS...")
...
def generate_summary_from_audio(client, audio_file_path):
    ...
    prompt = """
You are an elite audio briefing announcer. Analyze the provided podcast audio from "HBR IdeaCast" and create a highly condensed, intellectually dense 3-4 minute briefing script in English.
...
def main():
    ...
    state_file = os.path.join(os.path.dirname(__file__), "last_hbr_ideacast_guid.txt")
    ...
    if current_guid and os.path.exists(state_file):
        ...
        if current_guid == last_guid:
            print("No new HBR IdeaCast episode detected. Exiting.")
            return

    print(f"Latest Episode: {episode_title}")
    
    local_audio = os.path.join(os.path.dirname(__file__), "temp_hbr_ideacast.mp3")
    ...
    display_title = f"Harvard Business Review HBR IdeaCast: {episode_title}"
    full_text_for_feed = f"**{display_title}**\n\n" + summary_text
    
    import datetime
    date_str = datetime.datetime.now().strftime("%B %-d, %Y")
    spoken_text = f"This is Harvard Business Review, HBR IdeaCast. Episode of {date_str}. " + summary_text.replace("**", "")
