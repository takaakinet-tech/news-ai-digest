...
RSS_URL = "https://feeds.harvardbusiness.org/harvardbusiness/coldcall"

def fetch_latest():
    print("Fetching Cold Call RSS...")
...
def generate_summary_from_audio(client, audio_file_path):
    ...
    prompt = """
You are an elite audio briefing announcer. Analyze the provided podcast audio from "Cold Call" and create a highly condensed, intellectually dense 3-4 minute briefing script in English.
...
def main():
    ...
    state_file = os.path.join(os.path.dirname(__file__), "last_hbr_cold_call_guid.txt")
    ...
    if current_guid and os.path.exists(state_file):
        ...
        if current_guid == last_guid:
            print("No new Cold Call episode detected. Exiting.")
            return

    print(f"Latest Episode: {episode_title}")
    
    local_audio = os.path.join(os.path.dirname(__file__), "temp_hbr_cold_call.mp3")
    ...
    display_title = f"Harvard Business Review Cold Call: {episode_title}"
    full_text_for_feed = f"**{display_title}**\n\n" + summary_text
    
    import datetime
    date_str = datetime.datetime.now().strftime("%B %-d, %Y")
    spoken_text = f"This is Harvard Business Review, Cold Call. Episode of {date_str}. " + summary_text.replace("**", "")
